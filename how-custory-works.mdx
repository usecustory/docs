...
- A touchpoint shows where something happens in the journey. Example: a user reads the docs, starts onboarding, hits a paywall, contacts support, receives an email, or completes setup.
- An insight explains what the team learned. Example: users are confused by setup, customers do not understand pricing, or activation drops after the first integration step.
- An opportunity frames the problem worth solving. Example: make onboarding clearer, reduce billing confusion, improve the docs, or help customers reach value faster.
- A solution captures the proposed or shipped response. Example: rewrite the onboarding checklist, add an empty state, simplify pricing copy, create a help article, or change the product flow.
- A metric tells you whether the response worked. Example: activation rate, trial conversion, time to first value, support ticket volume, onboarding completion, retention, churn, or feature adoption.
