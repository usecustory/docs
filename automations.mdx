...
Automations keep your journey current without manual follow-up every time something important changes.

They help your team pull signals, create work, notify the right people, and refresh context on a schedule or in response to real events.
...
<AccordionGroup>
  <Accordion title="Automating before the journey data is current">
    Automations amplify the underlying data quality. Clean up statuses, ownership, and item quality first so the workflow is acting on signals your team already trusts.
  </Accordion>
